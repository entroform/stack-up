import { Animation, DOMImage, Easings, Num, Util } from '@nekobird/rocket';
    ...
    if (data.requireScale === true) {
      return new Animation({
        ...
        onTick: (n, ic, a) => {
          ...
          container.style.height = `${Num.modulate(
            n,
            1,
...
const appendItem = (src: string) => {
  return DOMImage.onImageLoad(src).then(() => {
...
const appendItems = () => {
  ...
  const promises = [];
  images.forEach(url => {
    const promise = DOMImage.onImageLoad(url).then(() => {
      const img: HTMLImageElement = <HTMLImageElement>document.createElement('IMG');
